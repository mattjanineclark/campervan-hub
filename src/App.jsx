...
function BookingList({bookings,dispatch,families,itineraries,onOpenItinerary,currentFamilyId}){
  const fColor=id=>families.find(f=>f.id===id)?.color??T.primary;
  const fName =id=>families.find(f=>f.id===id)?.name??"Unknown";
  const fEmoji=id=>families.find(f=>f.id===id)?.emoji??"";
...
function GuidesPanel({guides,dispatch}){
  ...
  const ICONS=["📖","🔑","🔌","💧","🔥","🚽","☀️","🛠️","⚠️","📄","🗺️","🔋","🧰","📷"];
  const handleFile=(e,isNew)=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const att={name:file.name,type:file.type,data:reader.result};if(isNew)setNf(f=>({...f,attachments:[...f.attachments,att]}));else setEf(f=>({...f,attachments:[...(f.attachments||[]),att]}));};reader.readAsDataURL(file);};
  ...
  const AttList=({atts,onRemove})=>atts?.length>0?(<div style={{marginTop:8}}>{atts.map((a,i)=>(
    <div key={i} style={{display:"flex",alignItems:"center",gap:8,background:T.bg,borderRadius:T.radiusSm,padding:"6px 10px",marginBottom:4,border:`1px solid ${T.border}`}}>
      ...
      <a href={a.data} target="_blank" rel="noreferrer" style={{fontSize:12,color:T.primary,textDecoration:"none",fontWeight:600}}>{a.type==="application/pdf"?"Open PDF":"View"}</a>
    ...
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",color:T.text}}>
      ...
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,position:"sticky",top:0,zIndex:50,boxShadow:"0 1px 12px rgba(45,106,79,0.08)"}}>
        <div style={{maxWidth:860,margin:"0 auto",padding:"0 12px"}}>
          ...
          {/* Row 2: nav dropdown left · Book Van right */}
          <div style={{display:"flex",alignItems:"center",gap:8,paddingBottom:10}}>
            <div style={{position:"relative",flex:1}}>
              <button onClick={()=>setMenuOpen(!menuOpen)}
                style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"8px 14px",background:T.bg,border:`1px solid ${T.border}`,borderRadius:T.radiusSm,cursor:"pointer",fontWeight:600,fontSize:13,color:T.text}}>
                <span>{currentTab.icon}</span>
                <span style={{flex:1,textAlign:"left"}}>{currentTab.label}</span>
                <span style={{color:T.textDim,fontSize:10,transform:menuOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>▼</span>
              </button>
              {menuOpen&&(
                <>
                  <div style={{position:"fixed",inset:0,zIndex:999}} onClick={()=>setMenuOpen(false)}/>
                  <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,background:T.surface,border:`1px solid ${T.border}`,borderRadius:T.radius,boxShadow:T.shadowLg,zIndex:1000,overflow:"hidden"}}>
                    {TABS.map((t,i)=>(
                      <button key={t.id} onClick={()=>{setTab(t.id);setMenuOpen(false);}}
                        style={{width:"100%",background:t.id===tab?T.primary+"12":"transparent",border:"none",borderBottom:i<TABS.length-1?`1px solid ${T.borderLight}`:"none",padding:"12px 16px",cursor:"pointer",textAlign:"left",color:t.id===tab?T.primary:T.text,fontWeight:t.id===tab?700:500,fontSize:14,display:"flex",alignItems:"center",gap:12,transition:"background 0.1s"}}
                        onMouseEnter={e=>{if(t.id!==tab)e.currentTarget.style.background=T.cardHover;}}
                        ...
                        <span>{t.label}</span>
                        {t.id===tab&&<span style={{marginLeft:"auto",color:T.primary,fontSize:13}}>✓</span>}
                      </button>
                    ))}
                  </div>
                </>
            ...
            <button onClick={()=>setShowBook(true)}
              style={{...btn(T.primary,T.surface,{padding:"8px 18px",fontSize:13,whiteSpace:"nowrap",flexShrink:0,boxShadow:`0 2px 8px ${T.primary}30`})}}>
              + Book Van
            </button>
          </div>
        ...
        {tab==="calendar"&&(
          <>
            ...
            <div style={card({padding:14})}>
              <UsageStats bookings={state.bookings} families={families}/>
            </div>
          </>
        )}
