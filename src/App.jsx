      ...
      <div style={{...card({padding:14,marginBottom:12})}}>
        ...
        <div style={{display:"flex",gap:8}}>
          {[["light","☀️ Light"],["dark","🌙 Dark"]].map(([m,label])=>(
            <button key={m} onClick={()=>onToggleTheme&&onToggleTheme(m)}
              style={{flex:1,padding:"10px",border:`2px solid ${themeMode===m?T.primary:T.border}`,
...
export default function App(){
  ...
  const [themeMode,setThemeMode]=useState(()=>{try{return localStorage.getItem("theme-mode")||"light";}catch(e){return"light";}});
